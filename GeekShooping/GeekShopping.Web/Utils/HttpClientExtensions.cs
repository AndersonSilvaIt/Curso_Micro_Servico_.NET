...
namespace GeekShopping.Web.Utils
{
    public static class HttpClientExtensions
    {
        ...
        {
            if (!response.IsSuccessStatusCode) throw new ApplicationException($"Something  went wrong calling API: {response.ReasonPhrase}");

            var dataAsString = await response.Content.ReadAsStringAsync().ConfigureAwait(false);

            var objectParse = JsonSerializer.Deserialize<T>(dataAsString,
